  ...
  const handleStatusChange = async (id, status) => {
    setCardColors(prevColors => {
      const updatedColors = {
        ...prevColors,
        [id]:
          status === 'deliver' ? CARD_COLORS.fullyPaid : CARD_COLORS.notPaid,
      };

      // ✅ Save updated colors to AsyncStorage
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tableHeader}>
        {['customerNameEng', 'billNo', 'finalAmount'].map((field, index) => (
          <TouchableOpacity
            key={field}
      ...
      {selectedTab === 'Sale' ? (
        ...
        <ScrollView style={styles.scrollArea}>
          {filteredRepairInvoices.length === 0 ? (
            ...
            filteredRepairInvoices.map((inv, index) => (
              <TouchableOpacity
                key={index}
                onPress={() => handleInvoicePress(inv)}
              >
...
const styles = StyleSheet.create({
  ...
  dropdownContainer: {
    position: 'absolute',
    top: 60, // Adjust this value to position the dropdown below the card
    left: 40,
